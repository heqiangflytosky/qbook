...
在这个例子中，我们会用到下面几个注解：
 ...
 - `@Injec`：用来标记依赖使用方：标记在需要注入依赖的变量， Dagger2 会帮助我们初始化。用来标记依赖的提供方：通过标记构造函数让 Dagger2 使用（Dagger2 通过 @Inject 可以在需要这个类实例的时候来找到这个构造函数并把相关实例 new 出来）从而提供依赖。
...
注意，使用这种方式时依赖使用方和依赖提供方的类必须要保持一致，上面的在Car中声明父类Engine是编译不过的。    
