...
适合使用离屏渲染的场景：     
 - 复杂 View 的alpha/translation/scale/rotation 动画
 - 需要应用 ColorFilter、Xfermode 等 Paint 效果
 - 绑制内容稳定，不频繁 invalidate


不适合使用离屏渲染的场景：       

 - 简单 View（TextView、Button、ImageView等）
 - 内容频繁变化的 View
 - 大尺寸的 View （接近全屏的 View）
 - 离屏渲染不适合永久开启（应该按需开启，用完关闭，比如动画期间）
