...
#### app 线程
surfaceFlinger 里面有个 app 线程，会分发 vsync。      

#### BufferTX

<img src="/images/android-performance-optimization-tools-perfetto/buffertx.png" width="897" height="194"/>

BufferTX：表示 SurfaceFlinger 接收到应用传来的 Buffer 并开始处理的时刻。     
TX 代表 "Transfer/Transmission"，即缓冲区的传输过程。    
SF 在每个合成节拍会尝试为目标 layer 取最新可用 Buffer。若某 layer 的 BufferTX 为 0，通常表示该 layer 暂无新 Buffer，SF 会沿用旧内容继续合成；对这个 App 来说表现为画面停滞/卡顿，但不代表 SF 全局“停止合成”。      
...
<img src="/images/android-performance-optimization-tools-perfetto/queuedbuffer.png" width="770" height="105"/>

QueuedBuffer 的最小值为 1，0 表示没有 Buffer 在队列中。      
QueuedBuffer 为 2 表示有 1 个 Buffer 在队列中，QueuedBuffer 为 3 表示有 2 个 Buffer 在队列中，以此类推。    

QueuedBuffer +1 的时机：    
RenderThread 执行 queueBuffer 操作时。      
RenderThread 将渲染完成的 Buffer 提交到 App 端的 BlastBufferQueue 中等待发送给 SurfaceFlinger。      

QueuedBuffer -1 的时机：    
收到 SurfaceFlinger 的 releaseBufferCallback。     
SurfaceFlinger 已使用完某个 Buffer，将其释放回 App 端 BlastBufferQueue，该 Buffer 可重新用于渲染。      
...
点击线程状态一行，可以在界面的下面看到 Thread State：
 ...
 - 浅绿色 : 可运行（Runnable）(R 状态)，代表线程可以运行但当前没有真正运行中，需要等待 cpu 调度，这个时间长短代表着cpu调度快慢。可以在Thread State看到当前线程唤醒者是谁。    
 - 白色/无色: 睡眠中（Sleeping）(S 状态)，当前线程没有工作可以做，等待事件驱动干活，比如looper就是大部分时间睡眠，小部分时间有消息后处理消息。     
 - 橙色Uninterruptible Sleep (IO)(D 状态)，代表不可以中断的休眠状态，一般线程在IO操作上阻塞了。不可中断状态实际上是系统对进程和硬件设备的一种保护机制。比如，当一个进程向磁盘读写数据时，为了保证数据的一致性，在得到磁盘回复前，它是不能被其他进程或者中断打断的。    
 - 紫红色Uninterruptible Sleep (Non IO)，不可中断的休眠状态，非IO导致，在等内核锁。通常是低内存导致等待、各种各样的内核锁。    

长时间的 D 状态意味着线程被完全阻塞，无法响应任何事件。如果发生在 UI 线程，极易导致 ANR。     
