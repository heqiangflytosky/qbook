...
enum InputConfig {
    ...
    // 此类窗口不支持在多个窗口之间拆分触摸事件。也就是说如果此窗口接收到了DOWN事件，
    // 那么后面的事件无论再不再窗口区域内都将发动到该窗口。
    ...
    // 此窗口希望在发生在其可触摸边界之外的 touch DOWN 事件时进行侦听。
    // 发生此类事件时，此窗口将收到带有 ACTION_OUTSIDE 的 MotionEvent。
    WATCH_OUTSIDE_TOUCH          = 1 << 9,

    // 表示事件可以跨窗口传递。允许手指移动到另一个窗口上方时触摸事件离开当前窗口开始在另外一个窗口传递。
    // 发生这种情况时，触摸刚刚离开的窗口（当前窗口）将接收ACTION_CANCEL，触摸已进入的窗口将接收ACTION_DOWN，
    // 触摸手势的其余部分将仅转到新窗口。
    ...
    //输入侦测窗口，此窗口将接收其可触摸区域内的所有事件，但不会阻止事件按 z 顺序发送到其下方的其他窗口。
    // 输入事件将分发到事件坐标处顶部非侦测窗口上方的所有侦测窗口。
    SPY                          = 1 << 14,
...
Inputflinger 是 input 事件的管理类，数据传递类，也是输入系统native层核心的模块。它包含了 `EventHub`、`InputReader`、`InputDispatcher` 模块。       
...
InputDispatcher 的实现主要涉及3个Queue，我们在 perfetto 中经常会看到：        
 ...
 - OutboundQueue：（“oq”）队列里面放的是即将要被派发给该目标窗口App的事件；每个窗口有一个独立的待发队列。用于暂存准备发给该窗口的事件，确保多事件顺序处理。    
                ...
                InputProcessor::notifyMotion
                    QueuedInputListener::flush()
                        InputDispatcher::notifyMotion() // 会将NotifyMotionArgs 转成 MotionEntry
                            // 将 MotionEntry 事件放到InputDispatcher mInboundQueue 队列中
        ...
        InputDispatcher::dispatchOnceInnerLocked
            ...
            InputDispatcher::dispatchMotionLocked // 只看 MotionEvent 的处理
                ...
                InputDispatcher::dispatchEventLocked // 发送事件到目标窗口InputTarget
                    ...
                    // 根据 InputChannel 获取 mToken
                    InputChannel::getConnectionToken() 
                    //根据 mToken 从 mConnectionsByToken 中找到对应的 Connection
                    InputDispatcher::getConnectionLocked() 
    ...
    public void setView(View view, WindowManager.LayoutParams attrs, View panelParentView,
        ...
        ......
                ...
                if ((mWindowAttributes.inputFeatures
                    ...
                    //如果当前窗口设置了 INPUT_FEATURE_NO_INPUT_CHANNEL，
                    // 代表当前窗口不希望接收输入事件，那么就不会走这里的创建流程
...
这一过程主要通过前面 `ViewRootImpl.setView` 中创建的 `WindowInputEventReceiver` 来实现。app 进程和 system_server 进程通过 socket 通信，底层捕获的事件最终通过 `inputChannel` 模块来传输，再由app端的 `WindowInputEventReceiver` 去接收，最后把事件分发到目标 `ViewRootImpl` 上。    
...
高触控采样率：这是最直接的硬件升级。普通屏幕的采样率为60Hz，而现在的旗舰手机普遍支持 240Hz 甚至 360Hz 的触控采样率。这意味着硬件每秒钟能采集更多次手指的位置信息，为后续处理提供更精细的原始数据。      
动态调节采样率：系统并非任何时候都需要最高采样率。通过 Touch Control HAL（硬件抽象层），系统可以在识别到游戏等高性能场景时，动态提高采样率以换取极致响应；在普通场景下则降低采样率以省电。      

2.内核与驱动层    

构建高效、标准的数据通道。     
这个阶段的目标是将硬件采集的数据快速、准确地传递给上层系统服务。    
使用Socket进行高效IPC：在Android 4.1之后，输入事件的跨进程传输方式从共享内存（Ashmem）改为了 Socket（套接字）。       
遵循标准协议：驱动层遵循标准的Linux 多点触控协议（MT Protocol） 来上报原始数据，并使用 SYN_REPORT 同步信号来标记一组完整的数据帧。这为上层 InputReader 的稳定解析提供了基础。      

3.系统服务层（Input系统）       
并行化分发：一个关键的变革是，InputDispatcher 不再等待应用确认上一个事件处理完成，就可以继续发送下一个事件。      
消费端批处理：与之配套的是应用端的批处理（Batching） 机制。InputDispatcher 可以一次性向应用发送多个待处理事件。应用端的 InputEventReceiver 在收到通知后，会在下一个VSYNC信号到来时，一次性消费掉这批事件。     
三重队列管理：InputDispatcher 内部通过 InboundQueue（待处理队列）、OutboundQueue（待发送队列）和 WaitQueue（等待队列） 来精细化管理事件流，既保证了高吞吐量，又通过超时机制（如应用5秒未响应则触发ANR）防止了事件无限堆积。      

4.应用框架层——与渲染机制完美对齐     
VSYNC对齐与优先级调度：应用端的 Choreographer（编舞者）机制确保了这一点。在每一帧开始，Choreographer 都会严格按照 CALLBACK_INPUT → CALLBACK_ANIMATION → CALLBACK_TRAVERSAL 的顺序执行任务。将 CALLBACK_INPUT 设为最高优先级，保证了用户的触摸输入永远是每一帧渲染的起点，从而让画面能最快地反映出手指的操作。      
处理历史点数据：对于开发者和应用来说，MotionEvent 提供的 getHistoricalX() 等API变得至关重要。通过遍历历史点，应用可以在一次绘制中画出完整的触摸轨迹，而不是依赖多个离散的、延迟的 ACTION_MOVE 事件。      
