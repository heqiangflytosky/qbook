...
```
adb pull /sdcard/perf_sysui.data ~/test/
```

进入 `android-sdk-linux/ndk/26.1.10909125/simpleperf` 文件夹，执行下面命令：     
```
python3.9 ./report_html.py -i ~/test/perf_sysui.data -o ~/test/perf_sysui.html
```

会自动打开perf_sysui.html，以火焰图的形式来统计各个方法的执行时长。      
